...
export default function ServicesPage() {
  ...
  const fetchServices = async () => {
    try {
      const response = await fetch(`/api/admin/salons/${params.id}/services`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingService
        ? `/api/admin/salons/${params.id}/services/${editingService.id}`
        : `/api/admin/salons/${params.id}/services`
  ...
  const handleDelete = async (serviceId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/salons/${params.id}/services/${serviceId}`,
        {
          method: 'DELETE',
        }
      )
  ...
  const handleToggleActive = async (service: Service) => {
    try {
      const response = await fetch(
        `/api/admin/salons/${params.id}/services/${service.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">Upravljanje uslugama salona</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {editingService
                  ? 'Izmeni detalje usluge'
                  : 'Dodaj novu uslugu za salon'}
              </DialogDescription>
            </DialogHeader>
